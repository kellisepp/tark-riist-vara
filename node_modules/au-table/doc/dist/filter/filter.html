...
}</code></pre></aubs-tab></aubs-tabset><p>Note how the text input is binding to the filter value with: <code>value.bind="filters[0].value"</code>. And the Select is doing the same thing: <code>value.bind="filters[1].value"</code>. With this approach you can add many filters very easily, just make sure you keep track of the filters index.</p><custom-filter></custom-filter></template>